...
class Item(object):
    ...
    def find_item(self):
        ...
        if int(tf.argmax(y_pred, axis=-1).numpy()) == 0:
            return ("티셔츠")
        elif int(tf.argmax(y_pred, axis=-1).numpy()) == 1:
            return ("바지")
        elif int(tf.argmax(y_pred, axis=-1).numpy()) == 2:
            return ("긴팔")
        elif int(tf.argmax(y_pred, axis=-1).numpy()) == 3:
            return ("드레스")
        elif int(tf.argmax(y_pred, axis=-1).numpy()) == 4:
            return ("코트")
        elif int(tf.argmax(y_pred, axis=-1).numpy()) == 5:
            return ("샌들")
        elif int(tf.argmax(y_pred, axis=-1).numpy()) == 6:
            return ("셔츠")
        elif int(tf.argmax(y_pred, axis=-1).numpy()) == 7:
            return ("스니커즈")
        elif int(tf.argmax(y_pred, axis=-1).numpy()) == 8:
            return ("가방")
        elif int(tf.argmax(y_pred, axis=-1).numpy()) == 9:
            return ("앵클부츠")
        else:
            return {"item": "인식불가"}
